...
export class JobService {
    ...
    async getJobs(paginationDto: PaginationDto): Promise<{ jobs: JobViewDto[], totalPages: number, page: number }> {
    ...
    async getPendingJobs(paginationDto: PaginationDto): Promise<{ jobs: JobViewDto[], totalPages: number, page: number }> {
        const { page = 1, limit = 9 } = paginationDto
        const skip = (page - 1) * limit
    ...
    async getOffersByJob(paginationDto: PaginationDto, id: number): Promise<any> {
        const { page = 1, limit = 9 } = paginationDto
        const skip = (page - 1) * limit
    ...
    async getPaymentsByJob(paginationDto: PaginationDto, id: number): Promise<any> {
        const { page = 1, limit = 9 } = paginationDto
        const skip = (page - 1) * limit
        try {
